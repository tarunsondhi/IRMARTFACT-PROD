...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/iminc.imblockchain.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml
  cd crypto-config/peerOrganizations/org2.imblockchain.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# and two Peer Orgs (``IMInc`` & ``Org2``) each managing and maintaining two peer nodes.
# This file also specifies a consortium - ``CryptoAnchor`` - consisting of our
...
# (``peer0.iminc.imblockchain.com`` & ``peer0.org2.imblockchain.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for IMIncMSP   ##########"
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/IMIncMSPanchors.tx -channelID $CHANNEL_NAME -asOrg IMIncMSP
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for IMIncMSP..."
    exit 1
  fi
